    ...
    socket.on("NotifyRollbackSeat", (msg) => {
        ...
        if (msg.owner === localStorage.getItem("UserCode")) {
            for (let i = 0; i < msg.rollBackSeat.length; i++) {
                console.log(`yes!!`);
                $(`#${msg.rollBackSeat[i]}`).removeClass("you-selected").addClass("not-selected");
                $(`#${msg.rollBackSeat[i]}`).attr("src", "../images/logo/icon_chair_not_selected.gif");
            }
        } else {
            for (let i = 0; i < msg.rollBackSeat.length; i++) {
                console.log(`OPPS`);
                $(`#${msg.rollBackSeat[i]}`).removeClass("selected").addClass("not-selected");
                $(`#${msg.rollBackSeat[i]}`).attr("src", "../images/logo/icon_chair_not_selected.gif");
            }
        }
    });
    ...
    socket.on("NotifyAddToCart", (msg) => {
        ...
        if (msg.owner === localStorage.getItem("UserCode")) {
            for (let i = 0; i < msg.addToCartSeat.length; i++) {
                $(`#${msg.addToCartSeat[i]}`).removeClass("you-selected").addClass("cart");
                $(`#${msg.addToCartSeat[i]}`).attr("src", "../images/logo/icon_chair_cart.gif");
            }
        } else {
            for (let i = 0; i < msg.addToCartSeat.length; i++) {
    ...
    function renderSeats(res) {
        ...
        for (let i = 0; i < res.data.length; i++) {
            if (parseInt(res.data[i].concert_area_seat_row) !== row) {
                $("#seats-table-list").append(`<tr id="row-${res.data[i].concert_area_seat_row}"></tr>`);
                row++;
            }
            ...
            $(`#${res.data[i].concert_seat_id}`).click({ param: countOfCartAndSold }, handleClick);
        }
    }
            ...
            .done(function (res) {
                $(function () {
                    ...
                    for (let i = 0; i < res.addToCartSeat.length; i++) {
                        $(`#${res.addToCartSeat[i]}`).removeClass("you-selected").addClass("you-cart");
                        $(`#${res.addToCartSeat[i]}`).attr("src", "../images/logo/icon_chair_cart.gif");
                    }
        ...
        .done(function (res) {
            $(function () {
                ...
                for (let i = 0; i < res.area_and_ticket_prices.length; i++) {
                    const concertAreaPriceId = res.area_and_ticket_prices[i].concert_area_price_id;
                    const { concert_area, ticket_price, total_seats } = res.area_and_ticket_prices[i];
