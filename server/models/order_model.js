...
const addToCart = async (chosenSeats, userId) => {
    ...
    try {
        ...
        await conn.query("UPDATE concert_seat_info SET status ='cart' where id IN (?)", [addToCartSeat]);

        await conn.query("INSERT INTO shopping_cart (concert_seat_id, status) VALUES ?", [insertData]);
...
const removeItemFromCart = async (deleteSeatId, userId) => {
    ...
    try {
        ...
        for (let i = 0; i < result.length; i++) {
            if (result[i].seat_status === "cart" && result[i].status_in_cart === "add-to-cart") {
        ...
        await conn.query("UPDATE shopping_cart SET status ='remove-from-cart' where  id = ? ", [
...
const checkout = async (data, user) => {
    ...
    try {
        ...
        let paymentResult;
        try {
            paymentResult = await payOrderByPrime(TAPPAY_PARTNER_KEY, data.prime, data.order, user);
        ...
        await conn.query("UPDATE concert_seat_info SET status ='sold' where id IN (?)", [orderSeatId]);

        // 將 shopping_cart table 中, 根據 shopping cart id 找到所對應到的座位，並將 shopping cart table 的 status 更改為 'remove-to-order'
        await conn.query("UPDATE shopping_cart SET status ='remove-to-order' where  id IN (?) ", [
        ...
        const [result] = await pool.query("INSERT INTO main_order SET ?", mainOrderRecord);
        ...
        await conn.query("INSERT INTO sub_order (main_order_id, shopping_cart_id) VALUES ?", [subOrderRecord]);
