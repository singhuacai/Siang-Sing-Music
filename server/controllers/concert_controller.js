...
const createConcert = async (req, res) => {
    ...
    const concert_info = {
        ...
        sold_start: moment(data.sold_start).subtract(offset_hours, "hours").format("YYYY-MM-DD HH:mm:ss"),
        sold_end: moment(data.sold_end).subtract(offset_hours, "hours").format("YYYY-MM-DD HH:mm:ss"),
    ...
    for (let i = 0; i < data.concert_info.length; i++) {
        ...
        for (let area = 0; area < data.concert_info[i].sku_info.length; area++) {
            ...
            const concertAreaPriceId = await Concert.insertConcertAreaPrice(concert_area_price);
            // console.log(concertAreaPriceId);

            let concert_seat_info = [];
            for (let row = 0; row < data.concert_info[i].sku_info[area].seat_allocation.length; row++) {
                for (let col = 0; col < data.concert_info[i].sku_info[area].seat_allocation[row]; col++) {
                    const seat = [
                        concertAreaPriceId,
...
const getConcertDetails = async (req, res) => {
    ...
    const [data] = result.map((v) => {
        v.sold_start = moment(v.sold_start).add(offset_hours, "hours").format("YYYY-MM-DD HH:mm:ss");
        v.sold_end = moment(v.sold_end).add(offset_hours, "hours").format("YYYY-MM-DD HH:mm:ss");
        v.concert_main_image = `/${concert_id}/${v.concert_main_image}`;
        v.concert_area_image = `/${concert_id}/${v.concert_area_image}`;
        v.concert_info[0].concert_datetime = moment(v.concert_info[0].concert_datetime)
